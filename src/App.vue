...
import { SITE_CONFIG, PROFILE_CONFIG, CUSTOMER_SERVICE_CONFIG, GLOBAL_BACKGROUND_CONFIG } from '@/utils/baseConfig';
...
export default {
  ...
  setup() {
    ...
    const globalBgStyles = computed(() => {
      const cfg = GLOBAL_BACKGROUND_CONFIG || {};
      if (cfg.enabled && cfg.imageUrl) {
        return {
          ...
          backgroundAttachment: cfg.attachment || 'fixed',
          minHeight: '100vh'
        };
      }
      return {};
...
.dark-theme .site-logo { background-color: var(--card-background); }
